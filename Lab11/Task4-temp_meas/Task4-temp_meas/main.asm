...
start:
	...
	ldi r16, 0x01	;set the mode for the ADC
	sts ADC0_CTRLA, r16

	ldi r16, 0x05	;set the prescaller
	sts ADC0_CTRLC, r16

	ldi r16, 0x0B	;set the analog input
	sts ADC0_MUXPOS, r16

	ldi r16, 0x01	;start the ADC
...
toggle_pin_ISR:
	push r16	;save reister values in stack
	in r16, CPU_SREG
	push r16
	push r17
	push r18
	...
	rcall multiplex_display	;call subroutine

	ldi r16, TCA_SINGLE_OVF_bm	;clear OVF flag
	sts TCA0_SINGLE_INTFLAGS, r16

	pop YL	;restore stack values to registers
	pop YH
	pop r20
	pop r19
	pop r18
...
analog_pin_ISR:
	push r16	;save register values in a stack
	in r16, CPU_SREG
	push r16
	push r17
	push r18
	...
	rcall signal_conv ;call subroutine

	pop YL	;restore stack valuues to register
	pop YH
	pop r20
	pop r19
	pop r18
...
signal_conv:
	...
	lds	mc16uL, ADC0_RESL	;set multi subroutine inputs
	lds	mc16uH, ADC0_RESH
	ldi	mp16uL, LOW(2500)
	ldi	mp16uH, HIGH(2500)
	rcall	mpy16u	;call 16 x 16 multiplex subroutine

	mov r18, r19;this shift the result 8 bits to the right
	mov r19, r20
	mov r20, r21
	ldi r21, 0x00

	ldi r22, 2	;shift the result 2 bits to the right
right_shift_loop:				
	lsr r19 ;shift another result right
	ror r18
	dec r22
	brne right_shift_loop

;Subtract 500 from the product
	ldi r24, HIGH(500)	;set negative 500 to two regiters
	ldi r23, LOW(500)

	sub r18, r23	;add which is subtracting in this case
	sbc r19, r24	;add with carry

	mov r16, r18	;prepare values for BCD2bin16 subroutine
	mov r17, r19

	rcall bin2BCD16	;call subroutine
